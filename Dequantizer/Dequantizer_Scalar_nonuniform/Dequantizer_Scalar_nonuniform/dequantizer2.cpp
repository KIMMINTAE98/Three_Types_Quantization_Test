...
bool Scalar_nonuniform_dequantization(string, string, string, string, int, int, int);
...
int main()
{
	...
	if (Scalar_nonuniform_dequantization(input_file, code_file, output_file, codebook_file, input_width, input_height, n_of_bits))
...
/* Scalar non-uniform dequantize input code file and reconstruct output image file  */
bool Scalar_nonuniform_dequantization(string input_file,     /* input image file     */
									  ...
									  int n_of_bits)         /* n-bits of code       */
{
	...
	if (fcb == NULL) {                                     // if input codebook file open fail, 
	...
	/* Decode codebook file to get reconstruct levels */ 
	...
	{                                                          // get reconstruction level
		R_Rlevels[i] = fgetc(fcb);                             //
		G_Rlevels[i] = fgetc(fcb);                             //  
		B_Rlevels[i] = fgetc(fcb);                             //
	}                                                          //
	fclose(fcb);                                               //
	...
	{                                                   //
		...
		{                                               //
			...
			{                                           //
				unsigned char rec = 0;                  // get reconstuction value
